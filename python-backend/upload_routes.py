from flask import Blueprint, request, jsonify, send_from_directory
import os
import pdfplumber
import docx
from werkzeug.utils import secure_filename
from dotenv import load_dotenv
import re

load_dotenv()

upload_bp = Blueprint('upload', __name__)
UPLOAD_FOLDER = "uploads"
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

ALLOWED_EXTENSIONS = {'.pdf', '.docx'}

# Fixed number of characters per page to avoid scrolling
CHARS_PER_PAGE = 1500  # Adjust to match your frontend's page height

def allowed_file(filename):
    return os.path.splitext(filename)[1].lower() in ALLOWED_EXTENSIONS

'''def paginate_text(full_text, chars_per_page=CHARS_PER_PAGE):
    """Split text into chunks without cutting words."""
    words = full_text.split()
    pages = []
    current_page = ""

    for word in words:
        # +1 for the space that will be added
        if len(current_page) + len(word) + 1 <= chars_per_page:
            current_page += (word + " ")
        else:
            pages.append(current_page.strip())
            current_page = word + " "

    if current_page.strip():
        pages.append(current_page.strip())

    return pages'''
# Fixed number of characters per page to avoid scrolling
CHARS_PER_PAGE = 1500  # Adjust as needed

def paginate_text(full_text, chars_per_page=CHARS_PER_PAGE):
    """
    Split text into pages without cutting sentences.
    Each page will end at a sentence boundary if possible.
    """
    # Split into sentences using regex (keeps ., ?, ! as delimiters)
    sentences = re.split(r'(?<=[.!?]) +', full_text)
    
    pages = []
    current_page = ""

    for sentence in sentences:
        # +1 for space when joining
        if len(current_page) + len(sentence) + 1 <= chars_per_page:
            current_page += sentence + " "
        else:
            # Finish current page
            pages.append(current_page.strip())
            current_page = sentence + " "

    # Add last page if not empty
    if current_page.strip():
        pages.append(current_page.strip())

    return pages

@upload_bp.route('/upload', methods=['POST'])
def upload_file():
    if 'file' not in request.files:
        return jsonify({"error": "No file uploaded"}), 400

    file = request.files['file']
    if not allowed_file(file.filename):
        return jsonify({"error": "Only PDF and DOCX files allowed"}), 400

    filename = secure_filename(file.filename)
    filepath = os.path.join(UPLOAD_FOLDER, filename)
    file.save(filepath)

    ext = os.path.splitext(filename)[1].lower()

    try:
        extracted_text = ""

        # PDF Handling
        if ext == '.pdf':
            with pdfplumber.open(filepath) as pdf:
                if len(pdf.pages) > 5:
                    os.remove(filepath)
                    return jsonify({"error": "PDF exceeds 5 page limit"}), 400

                full_text = "\n".join(
                    (page.extract_text() or "") for page in pdf.pages
                )
                extracted_text = paginate_text(full_text)

        # DOCX Handling
        elif ext == '.docx':
            doc = docx.Document(filepath)
            lines = [p.text for p in doc.paragraphs if p.text.strip()]
            approx_pages = len(lines) // 40
            if approx_pages > 5:
                os.remove(filepath)
                return jsonify({"error": "DOCX exceeds 5 page limit"}), 400

            full_text = "\n".join(lines)
            extracted_text = paginate_text(full_text)

        return jsonify({
            "message": "File uploaded successfully",
            "fileName": filename,
            "fileUrl": f"http://localhost:5000/uploads/{filename}",
            "content": extracted_text  # list of strings per page
        })

    except Exception as e:
        os.remove(filepath)
        return jsonify({"error": f"Error processing file: {str(e)}"}), 500


@upload_bp.route('/uploads/<filename>')
def uploaded_file(filename):
    return send_from_directory(UPLOAD_FOLDER, filename)
